...
/**
 * Shared seed dataset (Jamaica). Pure and browser-safe: the caller supplies an
 * id generator so the server (crypto random ids) and the static web demo
 * (deterministic ids) build the same shapes from one source — no drift.
 */
...
interface SeedSpec {
  ...
  cuts?: [number, number][]; // [price, daysAgo] oldest→newest
  firstPrice?: number;
...
/** Build the seed dataset using a caller-supplied id generator. */
export function buildSeed(makeId: (prefix: string) => string): SeedData {
  ...
  for (const s of SPECS) {
    ...
    listings.push({
      ...
      images: [0, 1, 2].map((n) => `https://picsum.photos/seed/${s.imageSeed}${n}/960/640`),
    ...
    for (const [price, daysAgo] of s.cuts ?? []) {
      priceHistory.push({ id: makeId("ph"), listingId: id, price, changedAt: daysAgoIso(daysAgo) });
    }
    ...
    for (let t = 1; t <= s.tier; t++) {
      verifications.push({
        ...
        method: VERIFICATION_METHODS[t] ?? "",
        verifiedBy: "Scam-Shield",
        verifiedAt: daysAgoIso(Math.max(1, s.listedDaysAgo - t)),
      });
    }
    // No comps seeded — jamaica has no MLS; Deal Score + Coach degrade honestly.
  }
